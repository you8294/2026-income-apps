...
├── 세금계산기.html                      ← 입주부담금 납부 계산기 (이주비 대출 조회)
├── analysis/
...
- 모든 페이지 푸터에 버전 표시 **`artistRDG vo2`** 통일 적용
  - `index.html`, `최종_괴정3_글자최대_출력최적화.html`, `최종_조합원_분양_혜택.html`, `세금계산기.html`, `analysis/building.html`
- `세금계산기.html` 추가 — 입주부담금 납부 계산기 (감정가·비례율 기반 납부 일정 및 이주비 대출 조회)
- `index.html`에 2번째 카드("입주부담금 납부 계산기") 추가
- 이전 저작권 문구("© 2026 괴정3가로주택정비사업조합") 삭제 완료
